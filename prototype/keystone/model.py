...
class Component:
    ...
    # UPPERCASE. This default read "assumption" (lowercase) while the comment beside it listed the
    # uppercase vocabulary, and `model_store._VALID_PROVENANCE` accepts only uppercase — so the
    # DEFAULT ingest path (`INGEST_PROVIDER=stub`, the $0 dev default) produced models that
    # simulate and render perfectly and then cannot be SAVED. Found while reviewing #21's model
    # store; the report/UI never noticed because they upper-case for display, which is exactly how
    # a value can be wrong for months while looking right everywhere a human looks.
...
class Assumption:
    ...
    # Same defect as `provenance` above, and found the same way: this default was NOT a member of
    # the vocabulary written in the comment beside it. `0001_canonical_model_store.sql:396` stores
    # `check (source in ('llm_inferred', 'benchmark', 'user'))`, so every Assumption built without
    # an explicit source was rejected the moment anyone tried to persist it.
    # `llm_inferred` is the right default because it is the one that CLAIMS LEAST: `benchmark`
    # would assert a measurement nobody took, and `user` would put words in their mouth.
    source: str = "llm_inferred"      # llm_inferred | benchmark | user
