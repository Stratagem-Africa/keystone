...
- Latency here treats Booking request queue as a SYNCHRONOUS hop — the queue's own wait is added to the request's latency as if the caller blocks on it. If the consumer is asynchronous (the usual reason to add a queue), real user-facing latency is lower than shown, and the backlog and drain time that actually matter are not modelled at all. The v1 engine has no async path; treat any flow through a queue as an upper bound.
