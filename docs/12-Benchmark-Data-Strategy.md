...
1. **Curation-discipline collapse — pressure to "look complete."** The temptation: accept a marginal citation, narrow a band, or mark `GROUNDED` to drive the assumption-count to zero. This single failure turns the effort into theatre. **Guardrails:** the type floor (no citation → can't construct); an explicit **banned-source list**; independent reviewer opens *every* link; a **published curation log** (what was considered/rejected and why); a **quarterly 10% audit** — if any audited citation doesn't resolve, the `curated` provider is downgraded to `GAP` until re-vetted. Failing direction is always `ASSUMPTION`/`GAP`, never a false `GROUNDED`. **Field evidence this risk is real (prior art):** a popular open-source peer — the *System Design Interview Simulator* ([github.com/vijaygupta18/system-design-simulator](https://github.com/vijaygupta18/system-design-simulator)) — advertises "verified / benchmark-backed" component specs in its README while shipping ~36 bare-literal numbers with **zero** in-tree provenance: the live "invented citation" trap. It is the concrete case for the type floor above — a value with no resolvable tag must be **structurally unable** to be `GROUNDED` (the corpus's provenance field is mandatory, never cosmetic).
...
## 8. Prior art — lessons from SQuADDS

[SQuADDS](https://github.com/LFL-Lab/SQuADDS) (USC LFL Lab; [Quantum 8, 1465 (2024)](https://quantum-journal.org/papers/q-2024-09-09-1465/)) is *"a validated design database and simulation workflow for superconducting qubit design."* Different domain (quantum hardware — transmons, resonators, Hamiltonians), but the **same shape as Keystone**: a validated database + a simulation workflow + prediction, taking you from target specs → a design. It's a useful mirror.

**What it confirms about this strategy:**
- **The database is the moat.** Their core asset is the *validated* design database (published openly on Hugging Face, peer-reviewed), not the simulator. That's the bet here: the grounded corpus is the thing that compounds.
- **Separate "measured" from "simulated/predicted," and validate one against the other.** They keep a `measured_device` dataset distinct from the simulated DB; "validated" *means* predicted-vs-measured agreement. That is exactly our `ASSUMPTION`-vs-`GROUNDED` split and the Charter's L1 loop.

**Concrete upgrades folded into this plan:**
- **Predicted-vs-measured discrepancy as the heart of calibration QA** (now in Phase 3). Their roadmap describes an "acceptance server" that computes the discrepancy between a contributed value and reality and routes it to maintainers — a good target shape for our calibration gate. A second, *production* instance of the same pattern: Alibaba's [Tair KVCache HiSim](https://github.com/alibaba/tair-kvcache) validates its predictor by trace replay and publishes the predicted-vs-measured gap as a per-scenario **MAPE** table (shape detailed in `docs/03` §4) — concrete evidence that discrepancy-as-QA is how an *earned* accuracy number is reported, not asserted.
- **A contribution pipeline + open corpus as a future.** Their `database/checker.py` ≈ our `validate_corpus.py`; their `contributor_*` subsystem + a web portal + a Hugging Face dataset turned the DB into a moat *and* a community-improved public good. Worth keeping as a future option once our corpus is real (with our QA gates intact).
- **Interpolation between measured points** (their `interpolations/`). Once we have grounded capacities at a few contexts, a future matcher could *interpolate an input capacity* for a nearby context (with a **widened band**) instead of only the current exact-match-or-`None`.
- **Plan the storage-scale path early.** Their wish-list is retrofitting "DataFrame → SQLite for larger-than-memory" as they scale; our JSONL → Postgres-behind-the-seam plan already anticipates this — keep the `ground()` seam clean so the swap is painless.

**The one line we must guard (and they don't have to):** SQuADDS's interpolation/ML **produces the design's numbers** — that *is* their product. Keystone's **prime directive forbids exactly that.** So we borrow their *mechanisms* (validated DB, discrepancy QA, contribution pipeline, even interpolation) but never their blurring of who-produces-the-number: any interpolation/ML here may only suggest or interpolate **grounded INPUT capacities** (with bands) — never a **derived metric** (bottleneck/breakpoint/latency/cost), which stays the engine's alone. (The simulation internals — Qiskit Metal / Ansys / Palace physics — don't transfer; it's the architecture and data strategy that map.)
