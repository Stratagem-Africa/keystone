...
- **Erasure:** deleting a project **hard-deletes** all its DB rows (`on delete cascade`). DB cascade does **not** reach Storage — so a project-delete also fires a cleanup function (Fly job / Supabase Edge Function) that **idempotently deletes every object under `{tenant_id}/{project_id}/*`, logging success/failure**, with a test asserting a deleted project leaves **zero** Storage objects. Append-only is *within* a project's life; a project delete purges it — no lingering cross-project retention.
  *(Amended 2026-09-21 — **proposed in PR #200, awaiting Bifola's ratification.** "Leaves **zero** Storage objects" is **best-effort at the DB layer**, not yet a guarantee. `keystone_delete_project` (migration 0006) returns only the uris that no surviving same-tenant row still references, because purging an object another document still points at would destroy that document's file — so a delete can deliberately leave a shared object in place. It cannot see other tenants' rows (RLS) or another transaction's uncommitted writes, and two concurrent deletes can deadlock (`40P01`, whole call rolls back, safe to retry). A failed purge is recorded in `DeletionResult.unpurged_uris` rather than lost. The durable guarantee — a purge-queue table written in the same transaction as the delete, or a per-uri reference count coordinated with the Epic 5.3 upload path — is **not built yet** and remains the requirement behind the zero-objects test and the kill criterion below.)*

### 6. Round-trips `model.py` exactly; validated, fail-closed, on write

The store persists the **runnable** shape the engine consumes so `ingest → store → load → simulate` is loss-less and the model stays the single source of truth. The `component` table mirrors the real `model.py:36–45` `Component` dataclass — **eight typed fields**: `id, kind, name, per_instance_rps, instances, base_latency_ms, monthly_cost_per_instance, provenance` (there is **no** `params` field today). A `params jsonb` column **may be added as a reserved, forward-compat bag for non-engine metadata only** — if added, a migration `CHECK` **rejects any key matching `(cost|capacity|latency|utiliz|throughput|breakpoint|bottleneck|rps|spof|headroom|confidence|p50|p95|p99|percentile|saturat)`** so a derived/engine field can never sneak in off the typed path (preserving §3). Likewise persist `flow.share`, `flow_step.visit_prob`, `workload` fields, the full `assumption[]`, and `system_model.domain_flags` — every `model.py` field, nothing dropped. *(The `params` forbidden-key list was widened 2026-08-10 — added `spof|headroom|confidence|p50|p95|p99|percentile|saturat` — to cover the full `SimulationResult` derived-field surface (`simulation.py:134-151`), so the guard and the engine's output field names stay in lockstep; surfaced by the #144 migration review, where the original 8-term list would have let a `p95`/`spof_count` key slip a derived value onto the typed path.)*
