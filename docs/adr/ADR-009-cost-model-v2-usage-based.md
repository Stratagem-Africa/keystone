...
**Status:** **Accepted** (2026-06-23 — Bifola: "all your recommendations") → build **Tier 1 now, Tier 2 next, defer the rest, on-prem out**. **Tier 1 (usage-based egress/storage/requests) is implemented** (PR #68, gated + adversarially reviewed; existing costs verified byte-unchanged). **Tier 2 part 1 — the reserved/spot discount lever — is implemented** (this PR): a single `compute_pricing` switch (on_demand=default / reserved_1yr / reserved_3yr / spot) applied to compute only, in pure-integer cents, with the report showing list → charged so the discount is never hidden; `on_demand` is a no-op so all existing numbers stay byte-unchanged. **Remaining Tier 2: AI/LLM per-token cost** (its own component kind + metric). **Grounding the usage rates + discount ratios with citations** (still ASSUMPTION seeds) is the tracked follow-up. Touches the engine's cost computation + model schema + money, so per CLAUDE.md it lands via branch → PR → review-gate, never self-applied unprompted.
