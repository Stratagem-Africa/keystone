...
**Status:** **Accepted** (2026-06-23 — Bifola: "all your recommendations") → build **Tier 1 now, Tier 2 next, defer the rest, on-prem out**. **Tier 1 (usage-based egress/storage/requests) is implemented** (PR #68, gated + adversarially reviewed; existing costs verified byte-unchanged). **Tier 2 part 1 — the reserved/spot discount lever — is implemented** (PR #69): a single `compute_pricing` switch (on_demand=default / reserved_1yr / reserved_3yr / spot) applied to compute only, in pure-integer cents, with the report showing list → charged so the discount is never hidden; `on_demand` is a no-op so all existing numbers stay byte-unchanged. **Tier 2 part 2 — AI/LLM per-token cost — is implemented** (this PR): per-component `llm_input_tokens_per_month` / `llm_output_tokens_per_month` × per-1K-token rates (output dearer than input) → an "ai" breakdown line, integer cents; zero tokens → no AI cost so non-AI models are byte-unchanged. Modelled the **cost only** via generic per-component token volumes (mirrors the Tier-1 usage shape); a first-class LLM `ComponentKind` with capacity/latency modelling is deferred. **This completes the ratified scope (Tier 1 + Tier 2; defer SaaS/licensing/support; on-prem out).** **Grounding the rates with citations** is now **PROPOSED** (`docs/14`, `benchmarks/grounded_pricing_rates.json`): all 8 rates researched + 3× adversarially verified against real vendor/aggregator pricing; the engine seeds now mirror the grounded centrals (notably `requests` corrected from a 3×-low guess) and a test locks them to the evidence — **pending Bifola's ratification of the citations** (AI proposes, a human ratifies grounded data; not self-merged), and not yet wired into the KB. Touches the engine's cost computation + model schema + money, so per CLAUDE.md it lands via branch → PR → review-gate, never self-applied unprompted.
