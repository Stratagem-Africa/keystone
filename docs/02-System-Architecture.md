...
- **Simulation Engine (`SIM`).** Deterministic. **v1 = analytical model** (queueing theory — Little's Law, **M/M/c** sojourn via Erlang-C over each tier's instance count, bottleneck propagation), *not* a full discrete-event simulator. It is **fully deterministic — there is no jitter and no seed**, because there is no randomness to seed: the same model always yields bit-identical output. Returns metrics with the formula that produced each one, plus a generated **derivation trace** (the engine's own deterministic steps, never prose) so the report shows its working. Confidence bands are **not** on this path: they are produced by `simulate_with_confidence`, and only for metrics whose inputs carry a cited range — a metric with no grounded input ships with no band rather than a fabricated one (`test_metric_envelope` pins that absence). (Rationale: do not try to out-engineer SysSimulator's Rust/WASM DES in v1; an analytical model is enough to validate designs and is cheap to build. DES is a v2 upgrade.) The concrete **v2 DES reference design** — a chronologically-sorted future-event calendar + a step loop, whose *replications* supply the **earned** confidence band the v1 heuristic only approximates — is documented in `docs/13` (Genesys-Simulator), which also validates that the engine stays the sole producer of numbers.
...
- **LLM-layer capability gating `SHOULD` (at activation).** When the council/ingestor go live, follow Genesys's pattern (`docs/13`): **deny-by-default capability flags** (network / model-mutation / execution off until explicitly granted), an **append-only, key-free AI audit log** of every model call, and **name-not-value** secret handling (config stores the env-var *name*, never the key). `simulate()` must never sit behind an LLM-grantable flag — the engine's authority to produce numbers is not a capability the LLM can be granted.
